S4_ATM_Transfer = ISPanel:derive("S4_ATM_Transfer")

function S4_ATM_Transfer:new(AtmUI, x, y, width, height)
...
function S4_ATM_Transfer:initialise()
...
function S4_ATM_Transfer:createChildren()
...
function S4_ATM_Transfer:render()
    ISPanel.initialise(self)

end

function S4_ATM_Transfer:setMsg(Msg)
...
function S4_ATM_Transfer:setTitleInfo(Title, Info)
...
function S4_ATM_Transfer:close()
    self:setVisible(false)
    self:removeFromUIManager()
end

-- panel move code
function S4_ATM_Transfer:onMouseDown(x, y)

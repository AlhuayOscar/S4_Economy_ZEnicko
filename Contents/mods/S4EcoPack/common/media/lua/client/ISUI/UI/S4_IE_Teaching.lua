...
function S4_IE_Teaching:createChildren()
    ...
    self.TraitOffers = {{
        ...
        name = teachingText("IGUI_S4_Teaching_Offer_BasicCombat", "Basic combat and maintanence"),
        id = "BasicCombatMaint",
        isMeleeExp = true,
        icon = "media/textures/S4_Icon/Icon_64_Jobs.png",
        description = teachingText("IGUI_S4_Teaching_Offer_BasicCombat_Desc", "+750 XP in all melee combat and maintenance skills.")
    }}
end
...
function S4_IE_Teaching.OnBuyTraitOfferStatic(data)
    ...
    local localApplied = true
    if offer.trait then
        localApplied = grantTraitToPlayer(player, offer.trait)
        if sendClientCommand then
    ...
    elseif offer.isMeleeExp then
        local xpSystem = player:getXp()
        if xpSystem and xpSystem.AddXP then
            pcall(function()
                xpSystem:AddXP(Perks.Axe, 750)
                xpSystem:AddXP(Perks.Blunt, 750)
                xpSystem:AddXP(Perks.SmallBlunt, 750)
                xpSystem:AddXP(Perks.LongBlade, 750)
                xpSystem:AddXP(Perks.SmallBlade, 750)
                xpSystem:AddXP(Perks.Spear, 750)
                xpSystem:AddXP(Perks.Maintenance, 750)
            end)
        end
    end
    ...
    if sendClientCommand then
        ...
        local logType = offer.trait and "Teaching Trait" or "Teaching Special"
        sendClientCommand("S4ED", "AddCardLog", {data.cardNumber, logTime, "Withdraw", price,
                                                 player:getUsername(), logType})
    end

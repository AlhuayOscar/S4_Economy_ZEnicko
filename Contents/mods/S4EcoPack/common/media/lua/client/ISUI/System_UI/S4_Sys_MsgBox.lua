S4_Sys_MsgBox = ISPanel:derive("S4_Sys_MsgBox")

function S4_Sys_MsgBox:new(SysUI, Px, Py, Pw, Ph)
...
function S4_Sys_MsgBox:initialise()
    ISPanel.initialise(self)
    
end

function S4_Sys_MsgBox:createChildren()
    ...
    local TitleW = getTextManager():MeasureStringX(UIFont.Small, self.SysUI.TitleName)
    TextMaxX = math.max(TextMaxX, TitleW + (S4_UI.FH_S*4))

    TextMaxX = Tx + TextMaxX + 20

    local BtnX = TextMaxX - 110
    self.OKBtn = ISButton:new(BtnX, Ty, 100, S4_UI.FH_S, getText("IGUI_S4_Com_Btn_OK"), self, S4_Sys_MsgBox.BtnClick)
...
function S4_Sys_MsgBox:BtnClick(Button)
...
function S4_Sys_MsgBox:onMouseDown(x, y)
...
function S4_Sys_MsgBox:onMouseUpOutside(x, y)
    if not self.Moving then return end
    self.SysUI.moving = false
end


function S4_Sys_MsgBox:close()

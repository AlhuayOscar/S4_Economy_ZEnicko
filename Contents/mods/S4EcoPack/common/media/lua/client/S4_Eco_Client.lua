...
local function reloadOpenShopWindows()
    local ComUI = S4_Computer_Main and S4_Computer_Main.instance or nil
    if not ComUI then return end

    local function safeReload(app)
        if not app or not app.isVisible or not app:isVisible() then return end
    ...
    safeReload(ComUI.GoodShop)
    safeReload(ComUI.GoodShopAdmin)
end
...
function S4_Eco_Client.OnReceiveGlobalModData(key, modData)
    ...
    if key == "S4_ShopData" or key == "S4_PlayerShopData" or key == "S4_ServerData" then
        reloadOpenShopWindows()
    end
end
Events.OnReceiveGlobalModData.Add(S4_Eco_Client.OnReceiveGlobalModData)

function S4_Eco_Client.OnConnected()
    if not SandboxVars.S4SandBox.SinglePlay and isServer() then return end
    -- Economey
...
function S4_Eco_Client.CreatePlayerData()
    ...
    if not PlayerModData and not PlayerShopModData and not PlayerXpModData then return end
...
function S4_Eco_Client.ComSpriteOn(Obj, Multi)
    if Obj:getSprite() == getSprite("appliances_com_01_72") then
        ...
        if Multi then Obj:transmitUpdatedSpriteToServer() end
                ...
                if Data.XYZCode then
                    ...
                    if square then
                        ...
                        for BoxNumeber, BData in pairs(BoxData) do
                            ...
                            if BData.Weight > 45 then BData.Weight = 45 end
...
function S4_Eco_Client.ShopAuthority()
    ...
    if not PlayerShopData then return end
    ...
    if NextBuyAuthority > 5 then NextBuyAuthority = 5 end
    ...
    if NextSellAuthority > 5 then NextSellAuthority = 5 end
    ...
    if BuyTotal >= BuyAuthorityTotal and SellTotal >= SellAuthorityTotal and BuyAuthority ~= NextBuyAuthority and SellAuthority ~= NextSellAuthority then

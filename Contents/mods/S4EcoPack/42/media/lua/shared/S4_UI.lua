...
local function S4_safeFontHeight(fontEnum, fallback)
    local textManager = getTextManager and getTextManager() or nil
    if not textManager or not textManager.getFontFromEnum or not fontEnum then
        return fallback
    end

    local ok, font = pcall(textManager.getFontFromEnum, textManager, fontEnum)
    if ok and font and font.getLineHeight then
        local okHeight, height = pcall(font.getLineHeight, font)
...
S4_UI.FH_S = S4_safeFontHeight(UIFont and UIFont.Small, 14)
S4_UI.FH_M = S4_safeFontHeight(UIFont and UIFont.Medium, 19)
S4_UI.FH_L = S4_safeFontHeight(UIFont and UIFont.Large, 24)

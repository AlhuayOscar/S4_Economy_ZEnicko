...
local function S4_shouldRenderDesktopIcon(iconKey)
...
function S4_Computer_Main:createChildren()
    ...
    for _, k in ipairs(orderedIcons) do
        if S4_Category.ComputerIconData[k] and S4_shouldRenderDesktopIcon(k) then
    ...
    for k, _ in pairs(S4_Category.ComputerIconData) do
        ...
        if not found and S4_shouldRenderDesktopIcon(k) then table.insert(renderedIcons, k) end
    end
